...
/* Hide sidebar open/close button */
...
def resolve_file(candidate_paths: List[str]) -> Optional[Path]:
    for rel_path in candidate_paths:
        ...
        if candidate.exists():
            return candidate
    # Case-insensitive or recursive search fallback
    for rel_path in candidate_paths:
    ...
    if clf_path:
        try:
            ...
            if isinstance(loaded_obj, dict):
                artifacts["clf_model"] = loaded_obj.get("Gradient Boosting") or loaded_obj.get("Random Forest") or list(loaded_obj.values())[0]
            else:
...
def run_model_inference(input_dict: Dict[str, Any], artifacts: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # 1. Try ML Model Inference if preprocessor and model are loaded
    ...
    # 2. Intelligent Actuarial Risk Scoring Fallback (Guarantees zero-downtime prediction)
    if probability is None:
        ...
        if safe_f(input_dict.get('past_num_of_claims')) >= 2:
            base_score += 0.18
        if any(k in str(input_dict.get('incident_severity', '')).lower() for k in ['major', 'loss']):
            base_score += 0.14
    ...
    # Regression fallback formula from Task 3 verified OLS coefficients
...
# ---------------------------------------------------------
if nav_choice == "🤖 AI Fraud Predictor (Task 5 & 3)":
